...
public class Details {
  public static void main(String[] args) {
    ...
    Integer startOfRange = input.nextInt();
    Integer endOfRange = input.nextInt();

    System.out.println(myBST.getCount(myBST.getRootNode(), startOfRange, endOfRange).number);
  }
}
...
class BinarySearchTree<T extends Comparable<T>>{
  ...
  public INT getCount(BinarySearchNode<T> root, T low, T high) {
    INT count = new INT();
    count.number = 0;
  ...
  boolean inRange(BinarySearchNode<T> root, T low, T high) {
    int compareResultOne = root.getData().compareTo(low);
    int compareResultTwo = root.getData().compareTo(high);
    return  compareResultOne >= 0 && compareResultTwo <= 0;
  }


  public boolean getCountUtil(BinarySearchNode<T> root, T lowerBound, T higherBound, INT count) {
    if (root == null)
      return true;
    boolean left = getCountUtil(root.getRightChild(), lowerBound, higherBound, count);
    boolean right = getCountUtil(root.getLeftChild(), lowerBound, higherBound, count);

    if (left && right && inRange(root, lowerBound, higherBound)) {
      ++count.number;
      return true;
    }
